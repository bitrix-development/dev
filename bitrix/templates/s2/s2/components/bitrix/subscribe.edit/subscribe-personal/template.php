...
<div class="subscription">
	...
	<div class="subscription-title">
		<?echo GetMessage("CT_BSE_SUBSCRIPTION_FORM_TITLE")?>
	</div>
				...
				<td class="field-form">
					<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
						<div class="subscription-rubric">
							<input type="checkbox" id="RUBRIC_<?echo $itemID?>" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> /><label for="RUBRIC_<?echo $itemID?>"><b><?echo $itemValue["NAME"]?></b><br><span><?echo $itemValue["DESCRIPTION"]?></span></label>
						</div>
					<?endforeach;?>
					...
					<div class="subscription-buttons"><input type="submit"  class="info-link " name="Save" value="<?echo ($arResult["ID"] > 0? GetMessage("CT_BSE_BTN_EDIT_SUBSCRIPTION"): GetMessage("CT_BSE_BTN_ADD_SUBSCRIPTION"))?>" /></div>
				</td>
			</tr>
		</table>
	</div>
	...
	<form style="margin-top: 20px;" action="<?=$arResult["FORM_ACTION"]?>" method="post">
	<?echo bitrix_sessid_post();?>
...
<input type="submit" class="info-link" value="<?echo GetMessage("CT_BSE_BTN_SEND")?>" />
	</div>
	</form>
	<?endif?>
